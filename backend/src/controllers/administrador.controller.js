...
export const updateAdmin = async (req, res) => {
	try {
		...
		const [result] = await pool.query(
			'UPDATE administrador SET user = IFNULL(?, user), password = IFNULL(?, password), nombre=IFNULL(?, nombre), apellido=IFNULL(?, apellido), correo=IFNULL(?, correo), celular=IFNULL(?, celular), pre_id=IFNULL(?, pre_id) WHERE cedula = ?',
			[
				user,
				password,
				nombre,
